...
当前处于把 prototype 做成可运行产品的本地开发阶段：三步引导 → 儿童四导航与风险回应 → 成人四导航和风险详情已可运行；成人端使用独立监护鉴权，只读聚合真实成长记录并隐藏完整普通聊天。本地风险详情只是明确未发送、不可确认的合成演练。邀请/同意/鉴权、AI 管线与基础安全护栏继续保留。**当前禁止向真实儿童开放或接入真实个人信息、真实通知。**
...
| 当前产品基线 | `AI陪伴智能体_PRD_V0.4_产品版.md`，状态快照更新于 2026-09-01；成人端概览/周报/提醒/设置/风险详情已接入独立监护鉴权和只读成长聚合。 |
| 工程包版本 | 根 `package.json` 为私有包 `xiaoban@0.1.0`；该值是工作区包版本，不是可验证的正式发布标签。 |
| 最近正式发布 | **没有可验证的正式发布。** 工作区不是 Git 仓库，没有 tag、commit 或 release 记录，也没有生产部署证据。`[待补充：首次正式发布版本号与日期；建议在初始化版本控制和发布流水线后，从 release/tag 与部署记录获取。]` |
...
| P2 | 版本控制 | 当前目录不是 Git 仓库，无法核验改动、分支、tag 或发布版本。 | `[待补充：远程仓库与分支策略；初始化/关联版本控制前先确认用户现有来源，不能擅自覆盖文件。]` |
...
- 当前工作区不是 Git 仓库。不要假设 `git status`、回退、分支或 tag 可用，也不要用破坏性 Git 命令处理用户文件。
