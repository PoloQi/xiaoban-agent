...
- 阶段6A.12（“未回执不显示成功”三处端到端走查，2026-09-18）：新增走查测试以真实契约parse→前端映射→降级判定的数据链锁定三处触点：儿童risk-sent改用共享常量`syntheticNotSentNoticeInput`（固定not_sent）；监护主页alerts契约锁死`notificationStatus:not_sent`/`acknowledgementStatus:unavailable`且契约直接拒绝delivered等成功状态；风险工作台open/not_sent、escalated+timed_out、单通道acknowledged均不显示成功，仅工单acknowledged/resolved/closed且双通道均acknowledged才显示本地回执并保留“不代表真实”；全仓用户侧文案grep无违规成功暗示。Web 7文件21/21、契约82/82、API离线119/119、全量MySQL集成25文件99/99、三工作区typecheck、Web构建通过；“未回执不显示成功”退出门槛按合成链路勾选，真实渠道回执接入后须重新走查；
- 阶段6A.13（“成人无法查看普通完整聊天”阶段级复核，2026-09-18）：纯只读/测试切片，新增跨切面MySQL集成测试三用例——①`information_schema`断言已迁移库无chat/conversation/transcript/message命名表（普通聊天请求-响应式不入库）；②儿童先发含独特探针句的聊天，监护人会话请求监护主页/风险工作台列表/数据权利导出三端点，响应体均不含探针句与聊天载体键，dashboard契约另锁定`ordinaryChatVisible:false`、`hiddenDetail:"完整普通聊天"`，导出仅排队无原文载荷；③监护人token调用`/api/v1/child/chat`返回403。新增MySQL集成1文件3/3、全量26文件102/102、API离线119/119、API typecheck通过；“成人无法查看普通完整聊天”退出门槛按当前无聊天持久化架构勾选，未来引入聊天入库必须重新走查；
- 阶段6B.2（“删除和审计链路通过”端到端走查与差距复核，2026-09-18）：纯测试切片，data-rights集成新增2用例——delete受理后被撤销儿童令牌实际请求成长端点返回403 ACCOUNT_DEACTIVATED（功能性删除端到端阻断而非只改字段），删除/导出审计与事件行经元数据白名单断言（仅枚举业务码，无联系方式/正文/聊天键），export写恰一条data_rights.export_queued审计；只读复核确认生产代码对audit_entries零UPDATE/DELETE、六张业务事件表均有禁改删触发器。data-rights集成6/6、全量MySQL集成26文件104/104、契约82/82、API离线119/119、Web 43/43、三工作区typecheck、API/Web构建通过；“删除和审计链路通过”退出门槛按合成边界勾选，阶段6五条门槛全部达成；真实导出文件、物理删除/保留策略评审、audit_entries数据库级禁改删加固列为真实试点前事项（须单独确认），不视为进入阶段7；
