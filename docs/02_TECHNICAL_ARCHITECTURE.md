...
阶段6B.2“删除和审计链路通过”端到端走查（2026-09-18，纯测试，无生产代码/契约/迁移变更）：

- `data-rights-service.integration.test.ts`在6B.1四用例基础上新增2用例：①delete经HTTP受理（201）后，同一儿童令牌请求`GET /api/v1/child/growth-plan`得到403 ACCOUNT_DEACTIVATED——功能性删除的五处写入（guardian_consents/enrollments/guardian_child_links/child_accounts/access_sessions）经各儿童服务authenticate的`revokedAt!==null || child.status!=="active"`检查端到端生效；该请求的audit_entries恰为`data_rights.functional_deletion_completed`+`child_account.deactivated`，事件恰为request_received/functional_deletion_completed，审计/事件metadata无phone/email/wechat/address/contact/text/message/conversation键且事件metadata恰为{requestType,reasonCode}；②export写恰一条`data_rights.export_queued`审计（此前只有代码、无断言），metadata恰为{requestType,reasonCode}，响应体无联系方式/正文载体。
- 差距复核（只读）：生产代码全仓对`audit_entries`零UPDATE/DELETE（仅内容工作流测试清理按actor_type删除）；六张业务事件表（safety overrides 006、generation control 007、risk ticket events 016、outbox claims 017、data-rights events 018、console notes 019）均有BEFORE UPDATE/DELETE触发器；身份撤回流程另写`guardian_consent.withdrawn`+`child_account.deactivated`审计。
- 遗留加固项（真实试点前，单独确认）：共用`audit_entries`表无DB级禁改删触发器，当前保障为应用层append-only+无管理员角色；真实导出文件生成/交付、物理删除与保留期限须责任主体与法律/伦理评审。
